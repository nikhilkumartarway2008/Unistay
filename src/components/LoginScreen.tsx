...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onNavigate, setUserRole }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    setLoading(true);
    try {
      const userProfile = await loginWithGoogle('STUDENT');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const userProfile = await login(phoneNumber, password);
      const r: UserRole = userProfile.role === 'OWNER' ? 'owner' : 'student';
      setUserRole(r);
      onNavigate(r === 'owner' ? 'owner-dashboard' : 'dashboard');
    } catch (err: any) {
